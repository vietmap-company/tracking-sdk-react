    ...
    <div className="flex h-[calc(100vh-52px)] flex-col">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-border bg-card px-4 py-2">
        ...
        <button
          ...
          onClick={() => {
            mapRef.current?.flyTo([105.85, 21.03], 12);
            log("flyTo Hà Nội");
          }}
        >
          Hà Nội
        ...
        <button
          className={toolbarBtn}
          onClick={() => {
            mapRef.current?.flyTo([108.2, 16.05], 12);
            log("flyTo Đà Nẵng");
          }}
        >
          Đà Nẵng
        ...
        <button
          ...
          onClick={() => {
            ...
            log(`getMembers → ${members.length} users`);
          }}
        >
          Log count
        </button>
        ...
        <div className="ml-auto">
          {lastEvent ? (
            ...
            <span className="text-xs text-muted-foreground">
              Zoom in để xem markers · click cluster để zoom
            </span>
          )}
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 p-3">
        <LiveMap
          ...
          onMapReady={(map) =>
            log(`Map ready — zoom ${map.getZoom().toFixed(1)}`)
          }
          onMapClick={([lng, lat]) =>
            log(`click ${lat.toFixed(4)}, ${lng.toFixed(4)}`)
          }
          onMarkerClick={(m: MemberStatus) =>
            log(`marker: ${m.name} (${m.statusLabel})`)
          }
        />
      </div>
    </div>
