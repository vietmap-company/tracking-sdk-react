...
export function PlaybackControls({
  ...
  const prog = total > 1 ? index / (total - 1) : 0;
  const pct = (prog * 100).toFixed(2);

  // Scrubbing via mouse/touch on the track
  const trackRef = React.useRef<HTMLDivElement>(null);

  const calcIndex = React.useCallback(
  ...
  const handleTrackMouseDown = React.useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
      const newIdx = calcIndex(e.clientX);
      onSeek(newIdx);
  ...
  const handleTrackTouchStart = React.useCallback(
    (e: React.TouchEvent) => {
      const newIdx = calcIndex(e.touches[0].clientX);
      onSeek(newIdx);

      const onMove = (ev: TouchEvent) =>
        onSeek(calcIndex(ev.touches[0].clientX));
      const onEnd = () => {
  ...
  return (
    <div className="absolute bottom-4 left-1/2 z-20 w-[min(480px,calc(100%-32px))] -translate-x-1/2 rounded-2xl border border-border/60 bg-card/90 p-3 shadow-elevated backdrop-blur-md">
      <div className="flex items-center gap-2">
        ...
        <div
          ref={trackRef}
          className="relative flex h-8 flex-1 cursor-pointer select-none items-center px-2"
        ...
        >
          ...
          <div className="relative h-1.5 w-full rounded-full bg-muted">
            ...
            {/* Thumb */}
            <div
              className="absolute top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-card bg-primary shadow-md"
              style={{ left: `${pct}%`, pointerEvents: "none" }}
            />
          </div>
        </div>
        ...
        <Tooltip>
          ...
          <TooltipContent>
            {autoFollow ? "Auto-follow: bật" : "Auto-follow: tắt"}
          </TooltipContent>
        </Tooltip>
      </div>
