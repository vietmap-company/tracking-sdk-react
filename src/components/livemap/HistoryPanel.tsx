...
import {
  ...
  Clock3, MapPin, Route, X, Navigation,
} from 'lucide-react'
...
export function computeSegments(pts: GpsPoint[]): Segment[] {
  ...
  while (i < pts.length - 1) {
    ...
    if (gap > GPS_LOST_MS) { segs.push({ type: 'lostGps', startTs: pts[i].time, endTs: pts[i + 1].time, durationMs: gap }); i++; continue }
    const type = pts[i].speed > 0 ? 'moving' : 'stopped'
    const start = i
    while (i < pts.length - 1 && pts[i].speed > 0 === pts[start].speed > 0 && pts[i + 1].time - pts[i].time <= GPS_LOST_MS) i++
    segs.push({ type, startTs: pts[start].time, endTs: pts[i].time, durationMs: pts[i].time - pts[start].time })
  }
  return segs
}
...
export function computeGroups(pts: GpsPoint[]): HistoryGroup[] {
  ...
  while (i < pts.length) {
    if (i > 0) {
      ...
      if (gap > GPS_LOST_MS) groups.push({ type: 'lostGps', startTs: pts[i - 1].time, endTs: pts[i].time, durationMs: gap })
    }
    if (pts[i].speed === 0) {
      const start = i
      while (i < pts.length && pts[i].speed === 0 && (i === start || pts[i].time - pts[i - 1].time <= GPS_LOST_MS)) i++
      const end = i - 1, dur = end > start ? pts[end].time - pts[start].time : 0
      if (end - start >= 2) groups.push({ type: 'stopped', startIdx: start, endIdx: end, pts: pts.slice(start, end + 1), durationMs: dur })
      else for (let j = start; j <= end; j++) groups.push({ type: 'moving', idx: j, pt: pts[j] })
    } else { groups.push({ type: 'moving', idx: i, pt: pts[i] }); i++ }
  }
  return groups
}
...
function TimelineBar({ pts, segments, totalMs }: { pts: GpsPoint[]; segments: Segment[]; totalMs: number }) {
  const { t } = useFleetwork()
  const first = pts[0], last = pts[pts.length - 1]
  const intervalMs = totalMs <= 2 * 3_600_000 ? 30 * 60_000 : totalMs <= 6 * 3_600_000 ? 60 * 60_000 : 2 * 3_600_000
  const ticks: { pct: number; label: string }[] = []
  const firstTick = Math.ceil(first.time / intervalMs) * intervalMs
  for (let tick = firstTick; tick < last.time; tick += intervalMs) {
    ticks.push({ pct: ((tick - first.time) / totalMs) * 100, label: fmtTimeShort(tick) })
  }
  const COLOR: Record<string, string> = { moving: STATUS_HEX.moving, stopped: STATUS_HEX.stopped, lostGps: STATUS_HEX.signal_lost }
  const totals: Record<string, number> = { moving: 0, stopped: 0, lostGps: 0 }
  segments.forEach(s => { totals[s.type] = (totals[s.type] ?? 0) + s.durationMs })
  ...
  return (
    <div className="px-4 py-3 border-b border-border/40 shrink-0 bg-muted/20">
      ...
      <div className="relative mb-1.5 h-4 text-[9px] text-muted-foreground/70">
        ...
        {ticks.map(tk => (
          <span key={tk.pct} className="absolute -translate-x-1/2" style={{ left: `${tk.pct.toFixed(1)}%` }}>{tk.label}</span>
        ))}
      ...
      <div className="flex h-5 overflow-hidden rounded-lg shadow-inner bg-muted/50">
        {segments.map((sg, i) => (
          <div
            ...
            style={{ width: `${Math.max((sg.durationMs / totalMs) * 100, 0.5).toFixed(2)}%`, background: COLOR[sg.type] }}
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {([['moving', t('history.moving')], ['stopped', t('history.stopped')], ['lostGps', t('history.lostGps')]] as [string, string][])
          .filter(([k]) => totals[k] > 0)
          .map(([k, lbl]) => (
            <span key={k} className="inline-flex items-center gap-1.5 rounded-full bg-background border border-border/50 px-2 py-0.5 text-[10px] font-medium text-foreground">
              <span className="h-2 w-2 rounded-full shrink-0" style={{ background: COLOR[k] }} />
...
function HistoryRowItem({ group, activeIndex, expanded, onToggle, onSeek }: {
  group: HistoryGroup; activeIndex: number; expanded: boolean; onToggle: () => void; onSeek: (idx: number) => void
}) {
  if (group.type === 'lostGps') {
    return (
      <div className="flex items-center gap-2.5 border-b border-border/30 bg-muted/30 px-4 py-2">
        <span className="h-2 w-2 rounded-full bg-slate-300 shrink-0" />
        <span className="text-[11px] italic text-muted-foreground">Mất GPS</span>
        <span className="ml-auto text-[10px] font-medium text-muted-foreground">{fmtDuration(group.durationMs)}</span>
      </div>
    )
  }

  if (group.type === 'stopped') {
    const hasActive = activeIndex >= group.startIdx && activeIndex <= group.endIdx
    return (
      <>
        {/* Stop group header */}
        <div
          ...
          className={cn(
            ...
            hasActive ? 'bg-amber-50 dark:bg-amber-950/20' : 'hover:bg-amber-50/50 dark:hover:bg-amber-950/10'
          )}
        >
          <div className={cn(
            'flex h-6 w-6 shrink-0 items-center justify-center rounded-full transition-colors',
            hasActive ? 'bg-amber-200 text-amber-700' : 'bg-amber-100 text-amber-600'
          )}>
            {expanded
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] font-semibold text-amber-700">Dừng {fmtDuration(group.durationMs)}</span>
            </div>
            <p className="text-[10px] text-muted-foreground mt-0.5 pl-4">{fmtTimeShort(group.pts[0].time)} → {fmtTimeShort(group.pts[group.pts.length - 1].time)}</p>
          </div>
          <span className="text-[10px] text-muted-foreground shrink-0">{group.pts.length} điểm</span>
        </div>
        {/* Expanded child rows */}
        {expanded && group.pts.map((p, pi) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn('font-mono text-[11px] tabular-nums', isActive ? 'font-semibold text-primary' : 'text-muted-foreground')}>
                {fmtTime(p.time)}
              </span>
              <span className="text-[11px] text-muted-foreground ml-auto">— km/h</span>
            </div>
          )
        })}
      </>
  ...
  return (
    ...
    >
      <span className={cn('h-2 w-2 shrink-0 rounded-full', isMoving ? 'bg-emerald-500' : 'bg-amber-400')} />
      <span className={cn('font-mono text-[11px] tabular-nums', isActive ? 'font-semibold text-primary' : 'text-muted-foreground')}>
...
export function HistoryPanel({ member, onClose, onHistoryLoaded, playIndex, onSeek }: HistoryPanelProps) {
  const { t } = useFleetwork()
  ...
  React.useEffect(() => { onHistoryLoadedRef.current = onHistoryLoaded }, [onHistoryLoaded])

  const loadHistory = React.useCallback(async (date: Date) => {
    setHistoryLoading(true); setHistoryPoints([]); setExpandedGroups(new Set())
    try {
      const startMs = new Date(date).setHours(0, 0, 0, 0)
      const endMs = new Date(date).setHours(23, 59, 59, 999)
      const pts = await LiveMapController.getHistoryRoute(member.userId, startMs, endMs)
      setHistoryPoints(pts); onHistoryLoadedRef.current(pts)
    } catch { setHistoryPoints([]); onHistoryLoadedRef.current([]) }
    setHistoryLoading(false)
  }, [member.userId])

  React.useEffect(() => {
    const today = new Date()
    setHistoryDate(today); setHistoryPoints([]); setExpandedGroups(new Set())
    loadHistory(today)
  }, [member.userId])

  React.useEffect(() => {
    const el = scrollRef.current?.querySelector(`[data-idx="${playIndex}"]`) as HTMLElement | null
  ...
  const toggleGroup = (gi: number) => setExpandedGroups(prev => {
    const next = new Set(prev); next.has(gi) ? next.delete(gi) : next.add(gi); return next
  })
      ...
      <div className="shrink-0 px-4 pt-4 pb-3 border-b border-border/40">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <span className={cn('mt-1 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium', STATUS_BADGE[member.status])}>
              <span className={cn('h-1.5 w-1.5 rounded-full', STATUS_DOT_PLAIN[member.status])} />
              {member.statusLabel}
            </span>
          </div>
          {/* Close */}
          <button type="button" onClick={onClose} className="shrink-0 flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
        ...
        {!historyLoading && pts.length === 0 && (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground leading-relaxed">Không có dữ liệu di chuyển<br />trong ngày đã chọn</p>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 divide-x divide-border/40 border-b border-border/40 shrink-0">
              <div className="flex flex-col items-center gap-0.5 py-3">
                ...
                <p className="text-[13px] font-bold text-foreground tabular-nums">{fmtDist(totalDistM)}</p>
                <p className="text-[9px] text-muted-foreground">{t('history.distance')}</p>
              </div>
              <div className="flex flex-col items-center gap-0.5 py-3">
                <Clock3 className="h-4 w-4 text-violet-500 mb-0.5" />
                <p className="text-[13px] font-bold text-foreground tabular-nums">{fmtDuration(totalMs)}</p>
                <p className="text-[9px] text-muted-foreground">{t('history.duration')}</p>
              </div>
              {/* GPS points stat removed */}
            </div>

            {/* Timeline */}
            {segments.length > 0 && <TimelineBar pts={pts} segments={segments} totalMs={totalMs} />}
