...
import type { GpsPoint, GpsUsersResponse, GpsUserRow, MemberStatus } from '@/lib/types'
...
function resolveStatus(code?: number): { status: MemberStatus['status']; statusLabel: string } {
  switch (code) {
    case 1: return { status: 'moving', statusLabel: 'Đang di chuyển' }
    case 2: return { status: 'stopped', statusLabel: 'Dừng' }
    default: return { status: 'signal_lost', statusLabel: 'Mất tín hiệu' }
  }
}
...
export const LiveMapController = {
  ...
  async getMember(userId: string, options: GetMembersOptions = {}): Promise<MemberStatus | null> {
    const members = await this.getMembers(options)
    return members.find((m) => m.userId === userId) ?? null
  },

  async getLastLocation(userId: string, options: { client?: AxiosInstance } = {}): Promise<GpsPoint | null> {
    const raw = await request<{ success: boolean; data: GpsPoint } | GpsPoint>(c(options), {
      method: 'GET',
  ...
  async getHistoryRoute(userId: string, startTime: number, endTime: number, options: { client?: AxiosInstance } = {}): Promise<GpsPoint[]> {
    const res = await request<{ trackingData: GpsPoint[] } | GpsPoint[]>(c(options), {
      method: 'GET',
