...
import type {
  ...
  MemberReportData, MemberStatusKind, MonthlyExpensesData, SummaryCardsData,
} from '@/lib/types'

export interface GetSummaryOptions { date?: number; client?: AxiosInstance }
export interface GetMemberReportOptions { page?: number; pageSize?: number; status?: MemberStatusKind; client?: AxiosInstance }
...
export const DashboardController = {
  ...
  async getMemberReport(date: number = startOfTodayMs(), options: GetMemberReportOptions = {}): Promise<MemberReportData> {
    return request<MemberReportData>(c(options), {
      method: 'GET', url: 'dashboard/gps-manager/users',
      params: { date, page: options.page ?? 1, pageSize: options.pageSize ?? 10, status: options.status },
    })
  },
