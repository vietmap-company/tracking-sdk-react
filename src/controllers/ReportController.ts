...
export interface BaseReportOptions {
  ...
  sortBy?: string;
  sortDesc?: boolean;
  /** Export format — when set, the response is a file blob instead of JSON. */
  format?: "excel" | "csv";
...
export const ReportController = {
  async getTripSummary(options: BaseReportOptions): Promise<TripSummaryReportData> {
    return request<TripSummaryReportData>(c(options), {
      method: "GET",
      url: "reports/trip/summary",
      params: buildParams(options),
    });
  },

  async getTripDetail(options: BaseReportOptions): Promise<TripDetailReportData> {
    return request<TripDetailReportData>(c(options), {
      method: "GET",
      url: "reports/trip/detail",
      params: buildParams(options),
    });
  },

  async getFuelSummary(options: BaseReportOptions): Promise<FuelSummaryReportData> {
    return request<FuelSummaryReportData>(c(options), {
      method: "GET",
      url: "reports/fuel/summary",
      params: buildParams(options),
    });
  },

  async getFuelDetail(options: BaseReportOptions): Promise<FuelDetailReportData> {
    return request<FuelDetailReportData>(c(options), {
      method: "GET",
      url: "reports/fuel/detail",
      params: buildParams(options),
    });
  },
